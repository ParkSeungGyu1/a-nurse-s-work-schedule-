  ...
  return (
    <div className="space-y-1.5">
      {constraints && constraints.length > 0 ? (
        constraints.map((c) => (
          <div key={c.id} className="flex items-center justify-between p-2 bg-muted/40 rounded text-xs" data-testid={`row-constraint-${c.id}`}>
            <div className="flex items-center gap-2">
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${CONSTRAINT_COLORS[c.constraintType] ?? ""}`}>
              ...
              {c.yearMonth && !c.date && <span className="text-muted-foreground">{c.yearMonth} 전체</span>}
              {c.isHard && <Badge variant="destructive" className="text-[10px] px-1 py-0">필수</Badge>}
      ...
      <Button size="sm" variant="outline" className="h-7 text-xs mt-1" onClick={() => setShowForm(true)} data-testid={`button-add-constraint-${nurseId}`}>
        <Plus className="w-3 h-3 mr-1" /> 요청 추가
      </Button>

      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-sm" data-testid="dialog-constraint-form">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              <Controller
                ...
                name="isHard"
                render={({ field }) => <Switch checked={field.value} onCheckedChange={field.onChange} data-testid="switch-is-hard" id="isHard" />}
              />
...
export default function RequestsPage() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto" data-testid="requests-page">
      <div className="flex items-center gap-3 mb-6">
        <div className="bg-primary/10 p-2 rounded-lg">
          <MessageSquare className="w-5 h-5 text-primary" />
        </div>
        <div>
          <h1 className="text-2xl font-bold tracking-tight">개인 근무 요청</h1>
          <p className="text-muted-foreground text-sm">간호사별 휴무 요청 및 근무 제약을 관리합니다.</p>
        </div>
      </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-start gap-4">
                  <div className="w-36 flex-shrink-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">{nurse.employeeNumber}</p>
                  </div>
