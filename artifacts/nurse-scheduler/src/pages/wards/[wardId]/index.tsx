...
export default function WardDetailPage() {
  ...
  const form = useForm<WardForm>({
    ...
    defaultValues: {
      name: "", wardType: "내과",
      shiftDStart: "07:00", shiftDEnd: "15:00",
      shiftEStart: "15:00", shiftEEnd: "23:00",
      shiftNStart: "23:00", shiftNEnd: "07:00",
    },
  });
  ...
  if (!ward) return <div className="p-6 text-muted-foreground">병동을 찾을 수 없습니다.</div>;

  return (
    <div className="p-6 max-w-2xl mx-auto" data-testid="ward-detail-page">
      <div className="flex items-center gap-3 mb-6">
        <div className="bg-primary/10 p-2 rounded-lg">
          <Building2 className="w-5 h-5 text-primary" />
        </div>
        <div>
          <h1 className="text-2xl font-bold tracking-tight">{ward.name}</h1>
          <p className="text-muted-foreground text-sm">병동 기본 설정</p>
        </div>
      </div>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select value={form.watch("wardType")} onValueChange={(v) => form.setValue("wardType", v)}>
                  ...
                  <SelectContent>
                    {WARD_TYPES.map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {[
              { label: "데이 (D)", startKey: "shiftDStart" as const, endKey: "shiftDEnd" as const, color: "bg-[hsl(var(--shift-d))]/20 text-[hsl(var(--shift-d))]" },
              { label: "이브닝 (E)", startKey: "shiftEStart" as const, endKey: "shiftEEnd" as const, color: "bg-[hsl(var(--shift-e))]/20 text-[hsl(var(--shift-e))]" },
              { label: "나이트 (N)", startKey: "shiftNStart" as const, endKey: "shiftNEnd" as const, color: "bg-[hsl(var(--shift-n))]/20 text-[hsl(var(--shift-n))]" },
            ].map(({ label, startKey, endKey, color }) => (
              <div key={label} className="flex items-center gap-4">
                <span className={`text-xs font-semibold px-2 py-0.5 rounded ${color} w-24 text-center`}>{label}</span>
