...
function getRecommendationTypeLabel(type: RecommendationItem["type"]) {
...
function getSuggestedShiftLabel(shiftType?: string) {
    ...
    <div className="flex h-full flex-col md:hidden">
      <div className="border-b bg-background px-3 py-3">
        <div className="mb-2 flex items-center justify-between gap-2">
          <div>
            <h2 className="text-sm font-semibold text-foreground">모바일 직접 보기</h2>
            <p className="text-[11px] text-muted-foreground">
              날짜 하나를 선택해 간호사별 근무를 빠르게 수정할 수 있습니다.
            </p>
          </div>
          {focusedDate && (
            ...
            >
              <div className="mb-2 flex items-center justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    카드를 누르면 근무가 D/E/N/OFF 순서로 바뀝니다.
                  </p>
                </div>
                {hasIssue && (
        ...
        <SheetHeader className="border-b px-4 pb-3 pt-2 text-left">
          ...
          <p className="text-xs text-muted-foreground">
            {mode === "recommendations"
              ? "부족 인원과 대체 후보, 공정성 경고를 모바일에서도 확인할 수 있습니다."
              : "검증 이슈를 날짜별로 빠르게 확인할 수 있습니다."}
          </p>
        </SheetHeader>
                ...
                <div key={item.id} className="rounded-xl border bg-white/90 p-3 shadow-sm">
                  ...
                  {(item.date || item.shiftType) && (
                    <div className="mt-2 flex flex-wrap gap-1.5 text-[10px] text-muted-foreground">
                      {item.date && <Badge variant="secondary">{item.date.slice(5)}</Badge>}
