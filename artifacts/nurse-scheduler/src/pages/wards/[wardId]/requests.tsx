    ...
    <div className="space-y-1.5">
      {constraints && constraints.length > 0 ? (
        <div className="flex flex-wrap gap-1.5">
          {constraints.map((c) => (
            <div key={c.id} className="flex items-center gap-1 rounded border bg-muted/40 p-1.5 text-xs" data-testid={`row-constraint-${c.id}`}>
              <span className={`whitespace-nowrap rounded-full px-1.5 py-0.5 text-[11px] font-medium ${CONSTRAINT_COLORS[c.constraintType] ?? ""}`}>
              ...
              {c.isHard && <Badge variant="destructive" className="px-1 py-0 text-[9px]">필수</Badge>}
              {c.notes && <span className="hidden text-muted-foreground sm:inline">· {c.notes}</span>}
              <Button
                size="sm"
                variant="ghost"
        ...
        <p className="text-xs text-muted-foreground">등록된 요청이 없습니다.</p>
      )}

      <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => setShowForm(true)} data-testid={`button-add-constraint-${nurseId}`}>
        <Plus className="mr-1 h-3 w-3" /> 요청 추가
      </Button>

      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="mx-auto w-[calc(100%-2rem)] max-w-sm" data-testid="dialog-constraint-form">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Label htmlFor="isHard" className="cursor-pointer text-sm">필수 제약으로 적용</Label>
            </div>

            <div>
              <Label>메모 (선택)</Label>
              <Input placeholder="예: 교육 일정" data-testid="input-constraint-notes" {...form.register("notes")} />
            </div>
...
export default function RequestsPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl p-4 md:p-6" data-testid="requests-page">
      <div className="mb-5 flex items-center gap-3 md:mb-6">
        <div className="rounded-lg bg-primary/10 p-2">
          <MessageSquare className="h-5 w-5 text-primary" />
        </div>
        <div>
          <h1 className="text-xl font-bold tracking-tight md:text-2xl">개인 근무 요청</h1>
          <p className="text-xs text-muted-foreground md:text-sm">간호사별 휴무 요청과 금지 근무를 관리합니다.</p>
        </div>
      </div>
