...
export default function RulesPage() {
  ...
  const form = useForm<RulesForm>({
    ...
    defaultValues: { maxConsecutiveWorkDays: 5, offDaysAfterConsecutiveWork: 2, maxConsecutiveNightShifts: 3, offDaysAfterNightShifts: 2, allowEToD: false, monthlyMaxNightShifts: 8, minExperiencedPerShift: 2, maxNewNurseRatioPerShift: 0.3, weekendFairness: true, holidayFairness: true },
  });
  ...
  function onSubmit(data: RulesForm) {
    upsertRules.mutate({ wardId, data }, {
      onSuccess: () => { queryClient.invalidateQueries({ queryKey: getGetWardRulesQueryKey(wardId) }); toast({ title: "규칙이 저장되었습니다." }); },
  ...
  function handleAddPair() {
    ...
    createPair.mutate({ wardId, data: { preceptorId: Number(newPair.preceptorId), precepteeId: Number(newPair.precepteeId), ruleType: "same_shift" } }, {
      onSuccess: () => { queryClient.invalidateQueries({ queryKey: getListPairRulesQueryKey(wardId) }); setNewPair({ preceptorId: "", precepteeId: "" }); toast({ title: "프리셉터 페어가 추가되었습니다." }); },
    });
  }
  ...
  if (isLoading) return <div className="p-4 md:p-6 space-y-4">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-40" />)}</div>;

  return (
    <div className="p-4 md:p-6 max-w-2xl mx-auto" data-testid="rules-page">
      <div className="flex items-center gap-3 mb-5 md:mb-6">
        <div className="bg-primary/10 p-2 rounded-lg">
          <Settings2 className="w-5 h-5 text-primary" />
        </div>
        <div>
          <h1 className="text-xl md:text-2xl font-bold tracking-tight">근무 규칙</h1>
          <p className="text-muted-foreground text-xs md:text-sm">스케줄 생성에 적용할 규칙을 설정합니다.</p>
        </div>
      </div>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            {[
              ...
              <div key={name} className="flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  <Input type="number" className="w-16 text-center h-8 text-sm" data-testid={`input-rule-${name}`} {...form.register(name)} />
                  <span className="text-sm text-muted-foreground w-5">{unit}</span>
                </div>
              </div>
            ))}
          </CardContent>
        ...
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-base">근무 배치 제한</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="flex items-center justify-between gap-4">
              <Label className="text-sm flex-1">경력 간호사 최소 인원 (근무 당)</Label>
              <div className="flex items-center gap-2">
                <Input type="number" className="w-16 text-center h-8 text-sm" data-testid="input-rule-minExperiencedPerShift" {...form.register("minExperiencedPerShift")} />
                <span className="text-sm text-muted-foreground w-5">명</span>
              </div>
            </div>
            <div className="flex items-center justify-between gap-4">
              <Label className="text-sm flex-1">신규 간호사 최대 비율 (근무 당)</Label>
              <div className="flex items-center gap-2">
                <Input type="number" step="0.05" min="0" max="1" className="w-16 text-center h-8 text-sm" data-testid="input-rule-maxNewNurseRatioPerShift" {...form.register("maxNewNurseRatioPerShift")} />
                <span className="text-sm text-muted-foreground w-5">비율</span>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-end gap-2">
            <div className="flex-1 w-full sm:w-auto">
              ...
              <Select value={newPair.preceptorId} onValueChange={(v) => setNewPair((p) => ({ ...p, preceptorId: v }))}>
                <SelectTrigger className="h-8 text-sm" data-testid="select-preceptor"><SelectValue placeholder="선임 선택" /></SelectTrigger>
